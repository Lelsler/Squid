...
# [5:end-1] forces 2005:2015 for tau values
df2 = dropmissing(DataFrame(XLSX.readtable("./DATA/PriceVolDataCorrected.xlsx", "Sheet1")...))
VolAll = Float64.(df2[:tons_DM][5:end-1]) ## CATCH DATA
PrAll = Float64.(df2[:priceMXNia_DM][5:end-1]) ## PRICE DATA

### New max time
tmax = length(df2[:Year][5:end-1]);
(Cmax, CmaxIdx) = findmax(VolAll);

# tau data is yearly from 2005:2015. Nothing else
expdata = CSV.read("tau.csv", datarow=1, header=["data"])
tau = expdata[:data]

# not sure which one to use here...
# extrema of tau.csv (mean of all data per year) (22.7850165, 71.19735287356322)
#minTau = 22.78
#maxTau = 71.19
# extrema of tau data (0.0391685, 110.0)
minTau = 0.04
maxTau = 110.0

q = 0.1*((tau-minTau)/(maxTau-minTau))
K = Cmax*q[CmaxIdx]
y_S = exp.(tau-maxTau)
...
function list_failures()
    ...
    for t in tmax-1
        if getvalue(S)[t+1] != getvalue(S)[t]+getvalue(g)*getvalue(S)[t]*(1-(getvalue(S)[t]/K))-getvalue(C)[t]
            println("- S[$(t+1)] ==> expected: $(getvalue(S)[t]+getvalue(g)*getvalue(S)[t]*(1-(getvalue(S)[t]/K))-getvalue(C)[t]), current: $(getvalue(S)[t+1])")
        end
    end
    for t in tmax
        if getvalue(C)[t] != q[t]*getvalue(E)[t]*getvalue(S)[t]
            println("- C[$(t)] ==> expected: $(q[t]*getvalue(E)[t]*getvalue(S)[t]), current: $(getvalue(C)[t])")
        end
    end
    if typeof(run) == Direct
        for t in tmax
        ...
        for t in tmax
            if getvalue(p_f)[t] != (getvalue(p_e)[t]-getvalue(c_p))*(1-R_tt[t])+R_tt[t]*getvalue(p_min)[t]
                println("- p_f[$(t)] ==> expected: $((getvalue(p_e)[t]-getvalue(c_p))*(1-R_tt[t])+R_tt[t]*getvalue(p_min)[t]), current: $(getvalue(p_f)[t])")
            end
        end
        for t in tmax
