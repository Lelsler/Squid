...
### Parameters #################################################################
b0 = -40.9079 # SST anomaly intersect
b1 = 0.020464 # SST anomaly trend
b2 = 0.165387 # SST anomaly period
b3 = - 0.287384 # SST anomaly period
l1 = -0.0059 # q, slope
l2 = 0.1882 # q, intersect
qc = 0.1 # catchability constant
a1 = 1/251266009027660.94 # migration trigger
...
for t in np.arange(0,tmax): # this assumes that by 2025 temperatures are high so that q = 0
     tau = translate(b0, b1, b2, b3, qc)

Tmin = min(tau)
Tmax = max(tau)

q = qc-(qc* ((tau-Tmin)/(Tmax-Tmin)))
...
for i in np.arange(0,1000):
    ye[i] = a1* np.exp(100*(b2*np.cos(xo[i])-b3*np.sin(xo[i])))
    if ye[i] > 0.9:
         xe[i] = xo[i]
...
### Define Model ###############################################################
def model(b0, b1, b2, b3, l1, l2, qc, Tmin, Tmax, Mmax, Mmin, a1, d, f, g, K, h1, h2, gamma, beta, c_p, c_t, flag):
    for t in np.arange(1,tmax):
        time = t + 2001
        tau[t]= b0 +b1 *(t+2001) +b2 *np.cos(t+2001) + b3 *np.sin(t+2001)

        # mantle length and catchability
        if mantle == 0:
            if ml[t] == 1:
                q[t] = qc - (qc* ((tau[t]-Tmin)/(Tmax-Tmin)))
            else:
                ML[t]= ml[t]
        ...
        if mantle == 0: # USE DATA INPUT?
            if ys[t] == 1: # run w/o data
                if any(time == xe):
                    ...
                    y_S[t] = Mmin # run with continuous function
            else:
        ...
        if flag == 1: #MLM
            # price for fishers
            p_f[t] = (p_e[t] -c_p) *(1-R_tt[t]) +R_tt[t] *((c_t *E[t])/C[t])
    ...
    return tau, ML, q, y_S, R_tt, S, E, C, p_e, p_f, RF, RT, G
...
##### Initiate arrays ##########################################################
sim = np.arange(0,1000) # number of simulations
...
for j in range(0,sim.shape[0]): # draw randomly a float in the range of values for each parameter
    ...
    for i in np.arange(1,tmax):
            tau, ML, q, y_S, R_tt, S, E, C, p_e, p_f, RF, RT, G = model(b0, b1, b2, b3, l1, l2, qc, Tmin, Tmax, Mmax, Mmin, a1, d, f, g, K, h1, h2, gamma, beta, c_p, c_t, flag)
            OUT1[i]= p_f[i]
            OUT2[i]= C[i]
            OUT3[i]= tau[i]
            OUT4[i]= y_S[i]
