...
### Define Model ###############################################################
def model(a0, a1, a2, a3, k, l, qc, delta, g, K, h1, h2, gamma, beta, kappa, sigma, alpha, lamda):

    L = translate(a0, a1, a2, a3, qc)

    Tmin = min(L) # Tmin(1990) = -0.43114117209719194
    Tmax = max(L) # Tmax(2015)= 0.79877341541962721

    q = qc-(qc* ((L-Tmin)/(Tmax-Tmin)))
    ...
    for t in np.arange(1,tmax):
        ...
        # migration of squid
        #M[t] = yo[t] # uses the fixed timeseries calc from 1990-2025 with initial parameters
        #M[t] = alpha *np.exp(lamda*(a2*np.cos(t)-a3*np.sin(t))) # discrete version
        if any(time == xe):
             #print time
             M[t] = Mmax
        else:
...
##### Run the model ############################################################
a1 = np.arange(0.0195,0.021,0.000075) # trens. steps to test a1 parameter
a3 = np.arange(0.1,0.5,0.02) # amplitude. steps to test a3 parameter
...
for i in np.arange(0,a1.shape[0]):
    for j in np.arange(0,a3.shape[0]):
        T, ML, q, M, R, S, E, C, p_m, p_f, I_f, I_t, G = model(a0, a1[i], a2, a3[j], k, l, qc, delta, g, K, h1, h2, gamma, beta, kappa, sigma, alpha, lamda)
