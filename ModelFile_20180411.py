...
#   tau[t]= b0 +b1 *(t+2000) +b2 *np.cos(t+2000) +b3 *np.sin(t+2000)
#   S[t]= 41.75 +(-5.696)* np.cos(t+2000) +16.397 *np.sin(t+2000)
#   y_S[t] = np.exp(tau[t]-(b0 +b1*(t+2000)))
...
def model(b0, b1, b2, b3, n1, n2, l1, l2, a1, g, K, m, f, B_h, B_f, h1, h2, gamma, beta, c_p, w_m, flag):
    for t in np.arange(1,tmax):
        ...
        # catchability
        q[t]= -0.0059 *tau[t] +0.1882
        # migration of squid
        y_S[t] = 0.0011642790824416532 *np.exp(tau[t]-(b0 +b1*(t+2015)))
        # trader cooperation
        R_tt[t] = (1-y_S[t])
        # squid population
